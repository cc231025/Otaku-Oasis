...
export default function StatusFilter(props) {
    // Based on the options object map through all possible Status Options and display them in the dropdown

    // Decide if the dropdown is open through the props useState

    const toggleDropdown = () => {
        (props.dropdownSelect !== 'status') ? props.setDropdownSelect('status') : props.setDropdownSelect('')
    }

    // Set the allOptions anew, if the option is already in there, remove it, otherwise reset it
    // Note only one Status can be selected at any time, since every anime has only one (finished, airing ...)
    ...
    return (
        <div className="relative inline-block text-left w-40 lg:w-52">
            ...
            {props.dropdownSelect === "status" && (
                <div
                    className="origin-top-right absolute left-0 w-52 z-20 bg-background_lighter rounded-md shadow-lg max-h-80 overflow-y-scroll">
                        ...
                        {options.status.map((option) => (
                            <div key={option.id} onClick={() => handleCheckboxChange(option)}
                                ...
                                <input
                                    ...
                                    checked={props.allOptions.status.includes(option)}
                                    readOnly={true}
