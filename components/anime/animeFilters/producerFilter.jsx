...
export default function ProducerFilter(props) {
    // Based on the options object map through all possible Producer Options and display them in the dropdown

    // Decide if the dropdown is open through the props useState


    const toggleDropdown = () => {
        (props.dropdownSelect !== 'producer') ? props.setDropdownSelect('producer') : props.setDropdownSelect('')
    }

    // Set the allOptions anew, if the option is already in there, remove it, otherwise reset it
    // Note only one Producer/Studio can be selected at any time, since every anime will most of the time have just one
    const handleCheckboxChange = (option) => {
        props.setAllOptions((prev) => ({
            ...prev,
            producer: prev.producer.includes(option)
                ? []
                : [option]
        }));
    };
    ...
    return (
        <div className="relative inline-block text-left w-40 lg:w-52">
            ...
            {props.dropdownSelect === "producer" && (
                <div
                    className="origin-top-right absolute right-0 w-[10rem] lg:w-[20rem] z-20 bg-background_lighter rounded-md shadow-lg max-h-80 overflow-y-scroll">
                    <div className="py-1 flex flex-wrap">
                        {options.producer.map((option) => (
                            <div key={option.id} onClick={() => handleCheckboxChange(option)}
                                ...
                                <input
                                    ...
                                    checked={props.allOptions.producer.includes(option)}
                                    readOnly={true}
